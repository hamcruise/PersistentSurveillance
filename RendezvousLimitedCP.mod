...
range Returns =1..ftoi(1+round(card(Task)/MaxChargingCnt));
...
dvar interval itvC2B[Returns][Chargers] optional size ChargingTime;

cumulFunction cumCharging[c in Chargers]= 
    step (0, MaxChargingCnt)
  - sum(t in Task:t.task=="charge")  stepAtStart(itvJ2C[t,c], 1, 1)
  + sum(r in Returns) stepAtStart(itvC2B[r,c], MaxChargingCnt, MaxChargingCnt);//ftoi(round(MaxChargingCnt/2))

dvar sequence seqCharger[c in Chargers] 
in 	  	append(all(t in Task) itvJ2C[t,c],
			   all(r in Returns) itvC2B[r,c])
...
dexpr float totDistance =
        ...
        Dist[t.n][typeOfNext(seqCharger[c], itvJ2C[t,c], t.n, t.n)]+
         sum(c in Chargers, t in Returns) 
        Dist[0][typeOfNext(seqCharger[c], itvC2B[t,c], 0, 0)]
        ;
...
forall (c in Chargers) cumCharging[c]<=MaxChargingCnt;
